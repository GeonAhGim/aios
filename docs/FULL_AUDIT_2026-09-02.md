...
| agent-platform-30 | exchanges bitget/kis | `_row_to_order` 파싱 — **완료 `67f23d0`** → **NH투자증권 어댑터 100% (사용자 지시)**: 스펙 `C:iose_nh_api_spec_v1.md` 완료 — 대상은 NAMUH PLUG OpenAPI(REST+WS)만, 레거시 QV(COM)는 범위 밖(PM 승인) → `src/exchanges/nh/` 구현 중(sandbox 미확인 시 `is_sandboxed=False`, 추정 엔드포인트는 파싱 실패 시 명시적 오류) → 이후 내구성 ①②④ → Bitget Demo e2e는 키 확보 시 | A → B |
