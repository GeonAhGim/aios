...
갱신 2026-09-12T17:04:32+00:00 · 총 23 · done 1 · inflight 22 · untouched 0 · hold 0 · 재오픈 1 · 깊이(done) {'D?': 1}

| ID | 리프 | 상태 | done task | 열린 task | depth | commit |
|---|---|---|---|---|---|---|
| AI-1 | gateway/contracts/v1.py + 스냅샷 | inflight |  | 2635 |  |  |
| AI-2 | domain/token_rules.py + domain/confirm.py + test | inflight |  | 2636 |  |  |
...
| AI-11 | experiments/application/{record,query,compare} + 통합 | inflight |  | 2646 |  |  |
| AI-12 | application/evaluate_proposal(검증 파이프라인 위임) + 통합 | inflight |  | 2647 |  |  |
| AI-13 | application/promote_to_paper(확인 토큰 + 리스크 게이트) + 적대적 테스트 | inflight |  | 2648 |  |  |
| AI-14 | application/research_tools(지표·백테스트·실험 위임) + test | inflight |  | 2649 |  |  |
| AI-15 | src/api/mcp/server.py + tools_read/research(얇은 프록시) + 적대적(인간 JWT 거부) | inflight |  | 2650 |  |  |
| AI-16 | tools_propose/paper + 확인 토큰 왕복 + 적대적(재사용 409) | inflight |  | 2651 |  |  |
| AI-17 | api/routers/ai.py(토큰 관리·제안·실험 조회) + 통합 | inflight |  | 2652 |  |  |
| AI-18 | ml/contracts + point_in_time + drift + test | inflight |  | 2653 |  |  |
| AI-19 | ml ports + parquet_feature_store + postgres_model_registry + 마이그레이션 | inflight |  | 2654 |  |  |
