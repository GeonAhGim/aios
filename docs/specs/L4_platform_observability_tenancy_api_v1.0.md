...
| `scripts/rotate_credential_keys.py` | 신규 | 회전 배치 | `--scope paper --from-kid v1 --to-kid v2 --batch 100 --dry-run`: `WHERE key_id = $from` 행을 `rewrap` 후 `UPDATE ... WHERE id=$1 AND key_id=$2`(105 조건부). 완료 후 `aios.security.key_rotation.count_total`, 감사 `security.key_rotated` | envelope | 120 |
