...
| 파일 경로 | 단일 책임 | 공개 계약 | 의존(포트) | 상한 | Zone |
|---|---|---|---|---|---|
| [신규] `src/services/oms/ports/repository.py` | Protocol 6개(OrderRepo, OrderEventRepo, FillRepo, OutboxRepo, InboxRepo, IdempotencyRepo) | 아래 어댑터의 시그니처를 Protocol로 | 없음 | 180 | SCAFFOLD |
...
| L4-31 | `application/{dispatch_outcome,outbox_commands,outbox_submit,outbox_writes}.py`(§2-C 소급 행), `core/safety/watchdog_simulator.py`(§2-H 소급 행, 실소유는 `L4_risk_and_safety_v1.0.md`); DEEPEN(task-2771): `tests/unit/oms/test_dispatch_outcome.py`, `tests/unit/core/safety/test_watchdog_simulator_deepen.py` | 14 | 5파일 전부 ≤300줄(`wc -l`); oms 4파일 docstring이 `L4-14`(원 구현 리프)와 `L4-31`(이 소급 행)을 함께 명시 + §4.2/§4.4 outbox 상태기계 표를 상호참조; `watchdog_simulator.py`는 §2-H 소급 행을 명시; 관련 기존 테스트 무수정 통과(`pytest tests/unit/oms/test_outbox_dispatcher.py tests/integration/test_watchdog_simulator.py`); DEEPEN — `dispatch_outcome.py`(순수 분류)와 `watchdog_simulator.py` 각각에 전용 테스트 파일 신설: ExchangeErrorKind 11종 전수 게이트, 레거시/미분류 예외 fail-closed negative, 예외 속성 조작 failure-injection, 상대예산 성능 하한, 스레드/asyncio 동시 호출 무간섭 proof(`pytest tests/unit/oms/test_dispatch_outcome.py tests/unit/core/safety/test_watchdog_simulator_deepen.py`) | 0(신규 코드 없음 — 문서 정합성만); DEEPEN 신규 테스트 2파일(운영 코드 변경 없음) |
...
갱신 2026-09-10T17:02:14+00:00 · 총 30 · done 30 · inflight 0 · untouched 0 · hold 0 · 재오픈 5 · 깊이(done) {'D0': 1, 'D2': 1, 'D3': 19, 'D?': 9}

| ID | 리프 | 상태 | done task | 열린 task | depth | commit |
|---|---|---|---|---|---|---|
| L4-1 | src/services/oms/contracts/{v1_commands,v1_events,v1_views}.py, domain | done | 101,2348,2753 |  | D3 | 6d57f292 |
| L4-2 | domain/state_machine.py, tests/unit/oms/test_state_machine.py | done | 101,1563,2753 |  |  | 6d57f292 |
| L4-3 | domain/idempotency.py, domain/rounding.py, 테스트 2 | done | 101,2753 |  |  | 6d57f292 |
| L4-4 | domain/symbol_registry.py, domain/venue_profile.py, exchanges/{bitget, | done | 2348,2753,2805 |  | D3 | d7aab1a1 |
| L4-5 | domain/fill_normalizer.py, domain/reconcile_rules.py, 테스트 | done | 101,2316,2753 |  |  | 6d57f292 |
| L4-6 | 마이그레이션 <rev1>(orders 컬럼·트리거·5개 테이블·REVOKE), tests/integration/oms/test | done | 2753,2762,2766 |  | D3 | b0b2920b |
| L4-7 | core/db/conditional_write.py에 extra_conditions 추가(기본값 없음, 기존 테스트 무변경), | done | 2754,2763,3536 |  | D3 | 13f805a7 |
| L4-8 | adapters/{outbox,inbox,fills,idempotency}_repository.py, 테스트 | done | 2753,2754,2764 |  | D3 | 6eeb0a53 |
| L4-9 | application/submit_order.py, order_service/submit.py 축소, order_service | done | 2753,2754,2765 | 3126 | D3 | 20f65102 |
| L4-10 | [FROZEN_PAPER_ONLY, PM 승인] core/executor/executor.py client id 교체 + st | done | 2351,2806 |  |  | f2dbfee1 |
| L4-11 | exchanges/common/{error_taxonomy,http_policy,rate_limiter,circuit_brea | done | 456,1551,2755 |  | D2 | eea98141 |
| L4-12 | exchanges/common/transport.py, bitget/error_codes.py, bitget/adapter.p | done | 1015,2315,2756 |  | D3 | 8f5c1b7e |
| L4-13 | exchanges/common/adapter.py ABC 확장(기본 구현), bitget/trading_mixin.py(fin | done | 1604,2316,2757 |  | D3 | 7f1b6ebe |
| L4-14 | application/outbox_dispatcher.py, wiring.py(디스패처만), test_outbox_dispat | done | 1567,2151,2759 |  | D3 | 1d402020 |
| L4-15 | application/inbox_processor.py, order_service/submit.py::apply_fill 위임 | done | 2183,2316,2761 |  | D3 | 096cc2b7 |
| L4-16 | application/unknown_resolver.py, order_service/reconcile.py 위임, test_u | done | 2151,2315,2769 |  | D3 | fdbffc28 |
| L4-17 | application/{cancel_order,modify_order}.py, order_service/{cancel,modi | done | 1603,2768 |  | D3 | d1ad4d7b |
| L4-18 | application/restart_recovery.py, execution_loop/recovery_wiring.py 연결, | done | 2151,2310 | 2673 | D3 | 436366d5 |
| L4-19 | exchanges/common/ws_session.py, bitget/ws_parsers.py 분할, bitget/market | done | 1551,2183,2760 |  | D3 | b459e0af |
| L4-20 | bitget/private_ws_mixin.py(orders/fill → inbox), wiring.py 구독 등록 | done | 2183,2797 |  | D3 | a87681cd |
| L4-21 | kis/adapter.py(transport·토큰 Lock·401 재발급), kis/trading_mixin.py(역조회),  | done | 2315,2799 |  | D3 | 23743a8f |
| L4-22 | exchanges/paper/{fill_model,fee_model,latency_model,venue_profile}.py, | done | 1534,1605,2758 | 2623 |  | dea42b01 |
| L4-23 | 마이그레이션 <rev3>, exchanges/paper/{ledger_repository,simulator_adapter}.p | done | 1605,2180,2770 |  |  | e958593a |
| L4-24 | application/three_way_reconciler.py, application/reconcile_scheduler.p | done | 2316,2800 |  |  | 0952b186 |
| L4-26 | application/order_query.py, 라우터는 별도 L4(API 명세) — 여기서는 서비스 함수까지 | done | 1602,2767 | 3542 | D3 | 13150db6 |
| L4-27 | 관측성: 메트릭 헬퍼 호출 삽입(§7.2 전부), 로그 필드(§7.3), tests/unit/oms/test_metrics_n | done | 2329,2801,3496 | 3381 | D3 | 09a985e6 |
| L4-28 | tests/perf/oms/ 4개(단언 포함) | done | 2329,2802,2804 |  |  | 7f9f6168 |
| L4-29 | tests/adversarial/oms/test_live_mode_bypass_attempts.py, test_tampered | done | 2180,2796 |  |  | 54ac90e3 |
| L4-30 | tests/e2e/bitget_demo/(키 확보 후), 프로파일 verified 갱신, 이 문서 §10 갱신 | done | 2179,2795 |  | D3 | 6536c34c |
| L4-31 | application/{dispatch_outcome,outbox_commands,outbox_submit,outbox_wri | done | 1758,2771 |  | D0 | 9684177c |
<!-- spec-status:end -->
