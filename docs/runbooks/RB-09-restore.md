...
`scripts/backup/{base_backup,wal_archive,restore_drill}.py`.
...
1. `python -m scripts.backup.base_backup --dest-dir <베이스 백업 상위 디렉터리>` —
   운영 DB에서 물리 베이스 백업 1회.
2. `python -m scripts.backup.wal_archive --verify-write <아카이브 디렉터리>` — 설정과
   실제 아카이빙 둘 다 검증. 실패하면 리허설을 진행해도 의미가 없다(다음 WAL부터
   끊겨 있으면 PITR이 그 시점 이후로 불가능하다) — 즉시 §"WAL 아카이빙이 죽어
   있을 때"로 간다.
3. `python -m scripts.backup.restore_drill --archive-dir <아카이브 디렉터리>` —
   1)의 최신 성공 백업을 별도 인스턴스로 복구하고 replay_verify까지 실행한다.
   cron/스케줄러로 하루 1회 돌리는 것이 전제다(`check_backup_drill`의 24시간
   기준과 맞춰야 한다).
4. 세 단계 모두 exit code 0이어야 그날의 리허설이 유효하다. `restore_drill.py`는
   실패해도 항상 임시 인스턴스를 내리고 임시 데이터 디렉터리를 지운다(운영
   인스턴스와 이름·포트가 겹치지 않게 `--restore-port`/`--restore-data-dir`를
   운영과 분리된 값으로 고정해서 돌린다).

## 복구 리허설이 실패했을 때

1. `C:\aios\pm\backup\drill_latest.json`(또는 로컬 `runtime/backup/drill_latest.json`)의
   `steps`를 본다 — 어느 단계에서 멈췄는지 그대로 남아 있다:
