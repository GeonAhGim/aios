...
## Status
Proposed (2026-09-26). 사용자 방향: "talib, pandas 같은 보조지표 모듈들을 아울러서 유연하고 다양하면서도
밀도 높은 차트 성능과 서비스를 제공하고 싶어" + "버전이 달라져도 유연하게 모두 쓸 수 있도록". 세 축 전부를
한 ADR로 올리라는 지시("전부 다 놓고 싶어", 산출물은 ADR 초안 먼저). 결정 대기: 사용자/CTO.
...
1. **aios-meta P6 300줄 가드 vs ADR-2026-09-10-C**: 어느 쪽이 정본인가. 가드가 정본이면 위 리프들은 파일 분할 비용을
   예산에 포함해야 하고, ADR이 정본이면 aios-meta 고정 커밋을 갱신해야 한다(사람 전용 저장소).
2. WASM 산출물을 저장소에 커밋(재현 해시 검증)할지, CI에서 매번 빌드할지. 제안: 커밋 + 해시 검증(CI에 Emscripten 불필요).
3. ichimoku류 비인과 지표를 "차트 표시 전용(causal=False)"으로 허용할지, 카탈로그에서 제외할지.
4. 1차 OSS 배치 55종의 우선순위 — 전부인가, 표시 빈도 상위부터인가(사용 데이터가 없어 이 문서는 정하지 않는다).
