...
## Decision 5 — 거래소 API 100% (2026-09-24 사용자 지시 재확인: Bitget·KIS·NH 우선 3사 + 추가 파이프라인)
2026-09-24 실측: KIS 353/375(94.1%, 미착수 0, 실전계좌필요 22), Bitget 132/156(84.6%, 미착수 23·P2 마진), NH 골격만
(8파일, 매트릭스 없음, get_order/get_ohlcv 미구현 — NH_GAPS.md). 추가 파이프라인은 SPI(`register_exchange_adapter_factory`)
·매트릭스 스크립트·계약 테스트 킷으로 존재하나 문서·자동 검사가 없다.
- "100%"의 정의(ADR-2026-09-06-I 유지): 매트릭스 `미착수 0`. `실전계좌필요`는 키 확보 후 왕복 검증으로 0, `범위밖`은 사유 필수.
- BR-16 Bitget 100%, BR-17 NH 매트릭스+계좌 불필요 전수, BR-18 거래소 추가 파이프라인(문서·SPI 검사 게이트·드라이런),
  BR-19 KIS 22건+NH 식별자 매핑 라이브 검증(S, 키 확보 후). 사람 항목: NH OpenAPI 키·KIS 모의투자 키를 .env에 등록.
- MVP-1 closeout 조건은 아니다(닫힘은 검증 우선) — LIVE 전환 조건에 포함한다.
