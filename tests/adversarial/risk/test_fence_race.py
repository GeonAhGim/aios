"""R-37 fence 경합 적대적 시나리오 — kill switch(fence++) vs 동시 제출.

Spec: docs/specs/L4_risk_and_safety_v1.0.md §9 R-37 DoD("gather 경합 post-fence 0"),
§3.6, §4.1 I1/I4, §10 cutover 행, 105번 동시성.
...
트리거 검증은 tests/adversarial/risk/test_orders_risk_decision_trigger.py 별도.
"""
...
async def test_staged_gather_kill_switch_vs_submits_post_fence_zero(
    ...
    async def early() -> Order:
        ...
        try:
            return await submit_with_fence(
                pool,
                adapter,
                make_order(ctx.execution_id),
                user_id=ctx.user_id,
                gate_decision=ctx.gate(),
                read_fences=ctx.read,
    ...
    async def late() -> Order:
        ...
        return await submit_with_fence(
            ...
            user_id=ctx.user_id,
            gate_decision=ctx.gate(),
            read_fences=gated_read,
            decision_reader=PostgresDecisionRepository(pool),
            metrics=metrics,
        )

    results = await asyncio.gather(
        *(early() for _ in range(_N_EARLY)),
        activator(),
        *(late() for _ in range(_N_LATE)),
