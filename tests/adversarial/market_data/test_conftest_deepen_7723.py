"""DEEPEN(task-7723) for tests/adversarial/market_data/conftest.py.

`conftest.py` only re-exports `pool` from
`tests.integration.foundation.market_data.conftest` (no new logic --
...
body (`asyncpg.create_pool` call + teardown `close()`) -- without
touching either conftest.py.
"""
...
from tests.integration.foundation.market_data.conftest import _asyncpg_dsn, pool
...
class TestPoolFixtureFailureInjection:
    ...
    async def test_pool_is_closed_on_teardown(self, monkeypatch: pytest.MonkeyPatch) -> None:
        ...
        cleanup would exhaust the pool's `max_size=16` connection budget
