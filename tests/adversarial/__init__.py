"""Adversarial test suite root package marker.

DEEPEN(task-7709), origin leaf task-6704 ("고아 산출물 회수"). This file has
no domain logic -- it is only the package marker that lets pytest treat
`tests/adversarial/**` as a package tree. The one real invariant it
carries is structural: every immediate subdirectory under it must itself
be an importable package (have its own `__init__.py`), or pytest silently
drops that subtree from collection instead of failing loudly -- exactly
the failure mode fixed ad hoc in task-6413 (950a960f, "add missing
__init__.py to unbreak pytest collection"). These tests pin that
invariant down with a reusable scanner instead of relying on someone
