"""Tests for the foundation integration-test package boundary.

The package initializer is intentionally small, but it is also the common
import boundary for every foundation integration test.  These checks keep
discovery fail-closed when a caller supplies an invalid package name or when
import metadata cannot be read.  Mirrors ``tests/foundation/unit/__init__.py``
(task-4084 DEEPEN precedent) for the integration package.
"""
...
class _PackageDiscoveryError(RuntimeError):
    """Raised when the integration-test package cannot be discovered safely."""
...
def test_integration_package_is_discoverable() -> None:
    """The foundation integration-test package must remain an importable package."""

    spec = _discover_package(__package__)

    assert spec.name == "tests.foundation.integration"
...
def test_discovery_rejects_a_module_spec_without_package_location(
    ...
    with pytest.raises(_PackageDiscoveryError, match="not importable as a package"):
        _discover_package("tests.foundation.integration")
...
def test_package_discovery_p95_is_within_budget(perf_budget: PerfBudget) -> None:
    ...
    samples = [
        s.cpu_ms
        for s in perf_budget.samples(
            lambda: _discover_package("tests.foundation.integration"), n=30
        )
    ]
