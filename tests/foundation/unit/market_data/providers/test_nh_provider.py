"""BR-20d-2 `adapters/providers/nh_provider.py` 계약 테스트.
...
`test_kis_provider.py`와 동일 패턴 — `NHAdapter` 생성은 네트워크 I/O가
...
def _listing(venue: Venue = Venue.NH_KRX, venue_symbol: str = "005930") -> VenueListing:
    return VenueListing(
        instrument_id=_ULID,
        venue=venue,
...
# ---- capabilities(): 실제 NHAdapter.get_capabilities()와 대조 -----------


def test_capabilities_matches_real_nh_adapter_declaration() -> None:
    real_adapter = NHAdapter(app_key="k", app_secret="s", act_no="12345678")
    exchange_capability = real_adapter.get_capabilities()

    caps = NHProvider(_FakeNHAdapter({})).capabilities()

    assert caps.provider_id == "nh"
    ...
    # NHMarketDataMixin.get_ohlcv는 일봉만 지원(market_data_mixin.py) — 그 이상을
    # 미리 선언하지 않는다(capability-gated 원칙).
    assert caps.timeframes == frozenset({Timeframe.D1})


def test_provider_structurally_implements_market_data_provider_protocol() -> None:
    assert isinstance(NHProvider(_FakeNHAdapter({})), MarketDataProvider)
...
async def test_fetch_candles_returns_candle_columns_filtered_to_span() -> None:
    fake = _FakeNHAdapter({"005930": [_candle(d) for d in range(5)]})  # 0..4일
    provider = NHProvider(fake)
...
async def test_fetch_candles_raises_coverage_missing_when_span_has_no_data() -> None:
    fake = _FakeNHAdapter({"005930": [_candle(d) for d in range(5)]})
    provider = NHProvider(fake)
...
async def test_fetch_candles_rejects_listing_from_other_venue() -> None:
    provider = NHProvider(_FakeNHAdapter({}))
    span = TimeSpan(start=_BASE, end=_BASE + timedelta(days=1))
    kis_listing = _listing(venue=Venue.KIS_KRX, venue_symbol="005930")

    with pytest.raises(ValueError, match="Venue.NH_KRX"):
        await provider.fetch_candles(kis_listing, Timeframe.D1, span)
...
async def test_fetch_candles_rejects_unsupported_timeframe() -> None:
    ...
    with pytest.raises(ValueError, match="NHProvider"):
        await provider.fetch_candles(_listing(), Timeframe.M1, span)
