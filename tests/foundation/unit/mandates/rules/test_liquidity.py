...
as:
  - failure injection: a `Decimal` subclass whose `__truediv__` raises makes
    `order_notional / adv` raise inside `check()`; wired through the real
    `RuleBundle`/`evaluate_bundle` gate (not `check()` called directly),
    this proves `_run_rule`'s fail-closed DENY (CM-A2) actually covers this
    rule — a corrupted numeric type that still passes the `isinstance(value,
    Decimal)` guard must never silently ALLOW.
...
class _ExplodingDecimal(Decimal):
    ...
    but raises on division — simulates a corrupted numeric type slipping
    past `liquidity._decimal_or_none`'s type guard."""

    def __truediv__(self, other: object) -> Decimal:
        raise ArithmeticError("simulated corrupted Decimal division")
...
def _bundle_of(params: dict[str, Any]) -> RuleBundle:
    return RuleBundle(
        version="test",
        rules=(RuleSpec(rule_id=liquidity.RULE_ID, params=params, check=liquidity.check),),
    )


def test_corrupted_decimal_division_fails_closed_through_the_gate() -> None:
    """실패 주입: `snapshot["order_notional"]`이 `isinstance(..., Decimal)`
    은 통과하지만 나눗셈 연산에서 예외를 던지는 손상된 서브클래스면
    `check()` 자신이 아니라 실제 게이트(`RuleBundle` + `evaluate_bundle`)가
    fail-closed DENY로 바꾸는지 증명한다(CM-A2)."""
    bundle = _bundle_of({"max_pct_of_adv": _MAX_PCT})
    snapshot = {
        "order_notional": _ExplodingDecimal("100100.00"),
        "average_daily_traded_value": _ADV,
    }
    ...
    assert decision.rule_hits[0].rule_id == liquidity.RULE_ID
...
def test_liquidity_check_meets_latency_budget_over_many_calls() -> None:
    ...
    for _ in range(10_000):
        liquidity.check(params, snapshot)
...
def test_liquidity_over_limit_turns_the_gate_red_and_under_limit_keeps_it_green() -> None:
    ...
    red = evaluate_bundle(
        bundle,
        {"order_notional": Decimal("100100.00"), "average_daily_traded_value": _ADV},
        now=_NOW,
    )
    ...
    green = evaluate_bundle(
        bundle,
        {"order_notional": Decimal("99999.00"), "average_daily_traded_value": _ADV},
        now=_NOW,
    )
...
def test_liquidity_rule_hit_rejects_post_construction_tampering() -> None:
...
def _replay_in_subprocess(params: dict[str, Any], snapshot: dict[str, Any]) -> tuple[str, str, str]:
    ...
    (spawn start method)."""
    hit = liquidity.check(params, snapshot)
    assert hit is not None
    return (hit.rule_id, hit.severity.value, str(hit.evidence))


def test_liquidity_replay_across_independent_processes_is_byte_identical() -> None:
