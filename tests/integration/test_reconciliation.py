...
async def _record(reconciliation, pool, *, symbol=None, exchange="bitget", discrepancy_pct=None):
...
async def test_single_discrepancy_only_triggers_auto_recovery(
    reconciliation, pool, circuit_breaker
):
    outcome = await _record(reconciliation, pool)
    assert outcome.action == "auto_recovery"
    state = await circuit_breaker.get_state()
    assert state.level == CircuitBreakerLevel.NORMAL


async def test_three_within_hour_escalates_to_restricted(reconciliation, pool, circuit_breaker):
    symbol = f"TEST{uuid4().hex[:8]}/USDT"
    for _ in range(3):
        outcome = await _record(reconciliation, pool, symbol=symbol)
...
async def test_five_within_day_escalates_to_halted(reconciliation, pool, circuit_breaker):
    symbol = f"TEST{uuid4().hex[:8]}/USDT"
    for _ in range(5):
        outcome = await _record(reconciliation, pool, symbol=symbol)
...
async def test_single_severe_discrepancy_immediately_halts(reconciliation, pool, circuit_breaker):
    symbol = f"TEST{uuid4().hex[:8]}/USDT"
    outcome = await _record(reconciliation, pool, symbol=symbol, discrepancy_pct=Decimal("15"))
...
async def test_different_symbols_counted_independently(reconciliation, pool):
    symbol_a = f"TESTA{uuid4().hex[:8]}/USDT"
    symbol_b = f"TESTB{uuid4().hex[:8]}/USDT"
    await _record(reconciliation, pool, symbol=symbol_a)
    await _record(reconciliation, pool, symbol=symbol_a)
    outcome_b = await _record(reconciliation, pool, symbol=symbol_b)

    assert outcome_b.count_1h == 1
    assert outcome_b.action == "auto_recovery"


async def test_repeated_severe_events_do_not_escalate_past_halted(
    reconciliation, pool, circuit_breaker
):
    symbol = f"TEST{uuid4().hex[:8]}/USDT"
    await _record(reconciliation, pool, symbol=symbol, discrepancy_pct=Decimal("20"))
    await _record(reconciliation, pool, symbol=symbol, discrepancy_pct=Decimal("25"))
