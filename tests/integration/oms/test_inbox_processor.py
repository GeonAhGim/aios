...
async def test_ingest_two_partial_fills_assigns_distinct_fill_seq_in_ledger(pool):
    """task-8080/F7 — `_process_row` must compute each fill's own `fill_seq`
    (venue_ts-ordered position among this order's fills), not hardcode 1 for
    every fill. `pos_journal.idempotency_key` is `f"fill:{order_id}:{fill_seq}"`
    (§5) — two partial fills sharing the same order must land as two distinct
    journal rows, `fill_seq=1` then `fill_seq=2`, not collapse into one."""
    user_id = await create_test_tenant(pool)
    execution_id = await _seed_execution(pool, user_id)
    order_id, client_order_id, exchange_order_id = await _insert_order(
        pool, user_id, execution_id=execution_id, quantity=Decimal("10")
...
async def test_duplicate_fill_seq_with_different_content_rejected_fail_closed(pool):
    """task-8080/F7 negative — if `fill_seq` computation ever regresses back to
    a hardcoded constant, two distinct fills would share one
    `pos_journal.idempotency_key`. The ledger must reject that as a digest
    mismatch (fail-closed), not silently overwrite the first fill's entry."""
