"""FA-4(963d5f3cfb1b) 마이그레이션 — ledger_journal_entry/ledger_posting_line WORM
회귀 + 원장 대차 불변 회귀.

Spec: docs/specs/L4_ibor_fund_accounting_and_resilience_v1.0.md#FA-4 DoD.
`ledger_journal_entry`/`ledger_posting_line`은 tenant_id 컬럼 자체가 없어 이
마이그레이션의 백필 대상 목록에 애초에 들어가지 않는다 — 컬럼 추가 후에도
NULL로 남는지만 확인한다(pos_journal WORM 가드 회귀는
`test_migration_fa4_worm_no_backfill.py`).
...
`hash_chain.verify_chain`(LC-3, `verify_ledger_integrity`가
TEST_DATABASE_URL 전체 저널에 대해 주기적으로 재계산)이 깨진 체인으로
오판할 위험이 있고, WORM이라 이 행은 절대 지울 수 없다 — 한 번 잘못 심으면
이 워커의 테스트 DB가 영구히 오염된다(실제로 이 파일의 이전 버전이 그렇게
오염시켜 `ledger_balance`가 없는 계정을 만들었고, 수동 복구가 필요했다).
`post_entry`는 해시체인·잔액을 모두 올바르게 채우므로 이 위험이 없다.

마지막 테스트들은 이 마이그레이션이 `ledger_posting_line`에 붙은 기존
DEFERRABLE 제약 트리거(`ledger_entry_balanced_trg`, Σ차변=Σ대변 강제)를
건드리지 않았음을 회귀로 확인하고, §6 실패 모드("감사 append 실패 → 포스팅
전체 롤백")를 실패주입으로 직접 유발한다."""
