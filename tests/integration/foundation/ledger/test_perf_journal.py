...
34.8ms/53.3ms/48.5ms — 목표(30ms)를 항상 초과한다. `append()` 한 번이
순차 DB 왕복을 약 7회 만든다(저널용 `pg_advisory_xact_lock` 1회 + "마지막
저널행" SELECT 1회 + 감사용 `pg_advisory_xact_lock` 1회 + "마지막 감사행"
SELECT 1회 + 감사 INSERT 1회 + 저널 INSERT 1회 + 분개행 INSERT 2회, 2행
엔트리 기준) — 배치·파이프라이닝이 전혀 없어 왕복 지연에 매우 민감한
구조다. 코드를 고쳐 왕복 횟수를 줄이는 것은 LC-17(테스트 전용 리프) 범위
밖이라(과제 지침 "프로덕션 코드 수정 금지") 손대지 않는다 — 아래 단언은
스펙이 요구하는 목표를 그대로 걸고, 실패 자체를 `xfail(strict=True)`로
고정해 조용히 넘기지 않는다."""
...
@pytest.mark.xfail(
    strict=True,
    reason=(
        "실결함(task-614/LC-17 발견, needs_decision): journal.append()이 호출당 "
        "~7회 순차 DB 왕복을 만들어 이 환경(aios_test_backend_3, localhost)에서 "
        "p95가 34~53ms로 목표(30ms)를 항상 초과한다. 모듈 docstring 참고 — "
        "왕복 축소는 프로덕션 코드 변경이라 이 테스트 전용 리프 범위 밖."
    ),
)
async def test_journal_append_p95_under_30ms(pool) -> None:
    ...
    print(f"\nledger journal append latency: p95={p95_ms:.3f}ms (n={len(latencies_ms)})")
