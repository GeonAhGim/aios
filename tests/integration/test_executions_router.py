...
async def client(event_bus):
    async with app.router.lifespan_context(app):
        ...
        # raise_app_exceptions=False — PLT-19가 executions.py의 raw
        # HTTPException을 ExecutionCreateError/ExecutionControlError/
        # CapitalAllocationError/CredentialNotFoundError로 교체했다. 도메인
        # 예외는 이제 전역 Exception 핸들러(ServerErrorMiddleware 승격)를
        # 거치는데, Starlette가 정상 응답 뒤에도 예외를 재전파하기 때문에
        # 필요하다(test_device_tokens_router.py client 픽스처와 동일 근거).
