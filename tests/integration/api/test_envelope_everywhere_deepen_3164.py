"""통합테스트 — task-3164 DEEPEN: PLT-17~21(task-1009) 봉투/예외매핑 실질 증빙.

원 리프(task-1009, `tests/integration/api/test_envelope_everywhere.py` +
`tests/integration/api/test_no_raw_http_exception.py`)는 marketplace/
strategy_builder/suitability 라우터에 대해 negative 테스트와 게이트
적색 재현(AST 탐지기 자체를 대상으로 한 negative test)은 있었지만,
실패 주입과 수치 성능 단언이 0건이라 D2 하한(ADR-2026-09-09-C
Decision 1) 미달이었다(task-3160 DEEPEN과 동일 결함 패턴, 대상 라우터만
다름).

이 파일은 원 파일들을 건드리지 않고(500줄 loc 래칫 회귀 방지,
ADR-2026-09-10-C §7) `GET /strategy-builder/strategies`(strategy_builder.py
`list_strategies`, `StrategyBuilderService.list_strategies` 단일 SELECT
왕복)를 대상으로 task-3160 DEEPEN과 동일한 세 가지를 새로 고정한다:
...
- 수치 성능 단언: ADR-2026-09-09-C Decision 1 예산표에 봉투 조회 전용
  ...
  하게 단일 실DB 왕복)을 자체 예산으로 차용한다(task-3148/3156/3160
  DEEPEN과 동일 차용 근거) — `StrategyBuilderService.list_strategies()`
  단일 SELECT 왕복 p95를 그 예산 내로 단언하고, 지연 주입으로 그 단언이
  실제로 깨짐을 재현한다.
...
class _ExplodingStrategyBuilderService:
    async def list_strategies(self, owner_user_id: object) -> object:
        raise _UnclassifiedDependencyFailure(
            "connection string: postgresql://admin:hunter2@internal-db/prod"
        )


async def test_unclassified_exception_is_enveloped_without_leaking_original_message(client: AsyncClient) -> None:
    ...
    app.dependency_overrides[get_strategy_builder_service] = lambda: (
        _ExplodingStrategyBuilderService()
    )
    try:
        response = await client.get("/strategy-builder/strategies", headers=headers)
    finally:
        del app.dependency_overrides[get_strategy_builder_service]
...
class _RawHttpExceptionStrategyBuilderService:
    async def list_strategies(self, owner_user_id: object) -> object:
...
async def test_raw_http_exception_with_unmapped_status_code_keeps_envelope_shape_and_status(
    ...
    headers = await _register_user(client)
    app.dependency_overrides[get_strategy_builder_service] = lambda: (
        _RawHttpExceptionStrategyBuilderService()
    )
    try:
        response = await client.get("/strategy-builder/strategies", headers=headers)
    finally:
...
# --- 수치 성능 단언: StrategyBuilderService.list_strategies() 단일 SELECT 왕복 p95 ---
...
# 특성을 가진다(task-3148 PLT-22, task-3156 PLT-07, task-3160 PLT-17~21
# DEEPEN과 동일 차용 근거).
...
async def _list_strategies_p95_ms(pool: asyncpg.Pool, user_id: uuid.UUID, *, n: int) -> float:  # noqa: ARG001
    ...
    for _ in range(n):
        ...
        await service.list_strategies(user_id)
        durations_ms.append((time.perf_counter() - start) * 1000)
    durations_ms.sort()
    return durations_ms[int(len(durations_ms) * 0.95)]


async def test_list_strategies_p95_under_borrowed_single_roundtrip_budget(pool: asyncpg.Pool) -> None:
...
async def test_list_strategies_budget_gate_fails_on_injected_regression(pool: asyncpg.Pool, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    p95_ms = await _list_strategies_p95_ms(pool, user_id, n=5)
