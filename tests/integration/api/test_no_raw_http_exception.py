"""PLT-17/PLT-18/PLT-19/PLT-20 — 이관된 라우터에 raw `HTTPException` raise가 없는지 정적 검사.
...
admin 등)는 여기 없다. 후속 리프(PLT-21)가 이관을 끝낼 때마다 이 목록에
파일을 추가해 검사 범위를 넓힌다.
"""
...
MIGRATED_ROUTERS: list[Path] = [
    ...
    Path("src/api/routers/executions.py"),
    Path("src/api/routers/portfolio.py"),
    Path("src/api/routers/reports.py"),
]
