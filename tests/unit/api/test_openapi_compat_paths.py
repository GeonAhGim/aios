"""PLT-16 — `check_openapi_compat.find_violations`(순수 JSON 비교) + CLI: path/method
레벨 위반 + 기본 CLI 동작.

DB·네트워크·앱 임포트 전부 없음(decision) — 베이스라인/현재 스키마는 이
파일에서 직접 만든 dict/fixture만 쓴다.

task-3940: 원래 test_openapi_compat.py(772줄)가 ADR-2026-09-10-C §7 500줄 기준을
넘겨 3개 파일로 분할됐다 — 이 파일은 그중 path/method 위반 + CLI 기본 동작
(1/3). 나머지는 test_openapi_compat_schemas.py(정규화·중첩 $ref, 2/3),
test_openapi_compat_actions.py(실패주입·성능·서브프로세스, 3/3).
"""
