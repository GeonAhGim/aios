...
스콥) — 해당 라우터 엔드포인트가 §3.3 봉투(`ApiResponse[...]`)를 쓰는지
회귀 가드.
...
wallet/exchange_credentials/foundation/paper_control·performance·
reconciliation·risk_gate·trust·validation 등 다른 레거시 라우터는 아직
raw HTTPException만 EXCEPTION_MAP으로 이관했을 뿐(또는 이관 예정일 뿐)
`ApiResponse[...]` 봉투로 응답을 감싸지 않는다(각자 도메인 모델을 직접
반환) — 그 전수 이관은 이 리프들의 DoD 밖이라 여기서 강제하면 바로
실패하는 거짓 기대를 심는 꼴이 된다. `src.main`을 통째로 import하지
않는 이유는 `tests/unit/api/contracts/test_handlers.py`와 동일(lifespan에
실제 secrets/DB pool 필요).
"""
...
from src.api.routers.foundation import connections, evidence, mandates
