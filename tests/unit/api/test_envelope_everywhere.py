"""PLT-21(admin.py 스콥) — admin 라우터 엔드포인트가 §3.3 봉투
(`ApiResponse[...]`)를 쓰는지 회귀 가드.
...
wallet/exchange_credentials/foundation/** 등 다른 레거시 라우터는 PLT-17~20이
raw HTTPException만 EXCEPTION_MAP으로 이관했을 뿐 아직 `ApiResponse[...]`
봉투로 응답을 감싸지 않는다(각자 도메인 모델을 직접 반환) — 그 전수 이관은
이 task-1074(admin.py만 담당, decision 참조)의 DoD 밖이라 여기서 강제하면
바로 실패하는 거짓 기대를 심는 꼴이 된다. `src.main`을 통째로 import하지
