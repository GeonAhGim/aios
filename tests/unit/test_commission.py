...
def test_calculates_commission_and_payout_at_default_rate() -> None:
...
def test_calculates_at_custom_rate() -> None:
...
def test_none_price_paid_returns_none_none() -> None:
...
def test_commission_plus_payout_equals_price_paid() -> None:
    price = Decimal("73.33")
...
def test_commission_delegates_rounding_to_domain_rounding_module() -> None:
...
def test_negative_price_raises_commission_error() -> None:
...
def test_rate_greater_than_one_raises_commission_error() -> None:
...
def test_rate_negative_raises_commission_error() -> None:
    """LC-2: 음수 수수료율은 의미없다 — 거부해야 한다."""
    with pytest.raises(CommissionError, match="Rate must be in \\[0, 1\\]"):
        calculate_commission(Decimal("100.00"), rate=Decimal("-0.10"))


def test_rate_nan_raises_commission_error() -> None:
...
def test_zero_price_returns_zero_zero() -> None:
...
def test_rate_exactly_one_returns_full_as_commission() -> None:
...
def test_rate_zero_returns_zero_commission() -> None:
...
def test_split_commission_exception_propagates_as_commission_error() -> None:
