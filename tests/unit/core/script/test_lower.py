...
def test_lowering_latency_p95_within_compile_budget_slice() -> None:
    ...
    자체의 지연만 분리해서 잰다. task-7434: 40ms 예산이 Windows
    `GetProcessTimes()` 틱(15.625ms)에 근접해 process_time 기반 perf_budget으로
    재면 결정적으로 넘겨 보일 수 있다 -- wall-clock perf_counter()를
    유지한다(coverage tracer는 그대로 정지)."""
