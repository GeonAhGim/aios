...
// task-1057 §3.3 5xx: useMe/useLogout(AppShell 의존)만 목으로 바꾸고 useListingSearch는
// 실제 훅(@aios/shared-hooks) 그대로 둔다 — useListingSearch가 곧바로
// {isError,error,refetch}를 흉내 내는 손조립 객체를 넘기면 컴포넌트가 routeApiError로
// 실제 판정하는지 검증할 수 없는 동어반복이 된다. 실제 apiClient가 진짜 fetch를 타도록
// fetch만 stub해 응답(상태코드·error_code)으로부터 재시도 가능 여부를 판단한다.
...
describe("MarketplaceBrowsePage 5xx 재시도 배선", () => {
  it("negative: EXCHANGE_UNAVAILABLE(503)은 api-client 자동 백오프(1s,2s) 소진 뒤에도 실패하면 다시 시도 버튼을 보여준다", async () => {
    vi.useFakeTimers();
    const body = {
      error_code: "EXCHANGE_UNAVAILABLE",
      message: "raw server detail",
      details: {},
      trace_id: "trace-mkt-503",
