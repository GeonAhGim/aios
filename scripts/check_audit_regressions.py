...
def check_optional_safety_gate() -> Finding | None:
    ...
    if hits:
        return Finding("optional_safety_gate",
                       "안전 게이트 인자가 Optional로 선언돼 있다 — 넘기지 않아도 통과한다(I-01 위반).", hits)
    return None
...
def check_require_mandate_false() -> Finding | None:
    ...
    if hits:
        return Finding("require_mandate_false",
                       "운영 조립 지점이 require_mandate=False로 게이트를 만든다 — 컴플라이언스 판정이 무력하다.", hits)
    return None
...
def check_freshness_tracker() -> Finding | None:
    ...
    if body and "freshness_tracker" not in body:
        return Finding("freshness_tracker_unwired",
                       "main.py가 DataFreshnessTracker를 만들지 않는다 — 서킷브레이커가 기동 직후 영구 HALTED가 된다.",
                       ["src/main.py"])
    return None
...
def check_xfail_in_gate_tests() -> Finding | None:
    ...
    hits = [h for h in hits if "xfail 없음" not in _read(ROOT / h.split(":")[0]).splitlines()[int(h.split(":")[1]) - 1]]
    if hits:
...
def check_tenant_fk_to_users() -> Finding | None:
    ...
    if hits:
        return Finding("tenant_fk_to_users",
                       "tenant_id가 users(user_id)를 FK한다 — 실제 테넌트 테이블은 `tenant`다(ADR-2026-09-06-G D0).", hits)
    return None
...
def check_rls_enable_without_force() -> Finding | None:
    ...
    if missing and "__loop__" not in forced:
        return Finding("rls_enable_without_force",
                       f"RLS를 ENABLE만 하고 FORCE하지 않은 테이블: {', '.join(missing)} — 소유자 연결이 정책을 우회한다.",
                       missing)
    return None
...
def check_duplicate_type_names() -> Finding | None:
    ...
    if out:
        return Finding("duplicate_type_names",
                       "같은 이름의 계약 타입이 둘 이상 정의돼 있다 — 잘못된 쪽을 임포트해도 타입검사를 통과한다.", out)
    return None
