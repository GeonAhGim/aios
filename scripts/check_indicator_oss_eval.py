"""CLI gate for IND-9 INDICATOR_OSS_EVAL.md (task-2919 deepen).

Reads the eval markdown and pyproject.toml, runs `assert_oss_eval_gate`.
...
from src.core.indicators.oss_eval_gate import (  # noqa: E402
    OssEvalGateError,
    assert_oss_eval_gate,
)

_DEFAULT_EVAL_PATH = _REPO_ROOT / "docs" / "design" / "INDICATOR_OSS_EVAL.md"
...
def main(argv: list[str] | None = None) -> int:
    ...
    except OssEvalGateError as exc:
        print(f"FAIL: {exc}")
        return 1
    print(
        f"OK: excluded={sorted(report.excluded_packages)} "
        f"talib_declared={report.talib_declared} "
