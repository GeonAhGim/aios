...
`core.portfolio.models` MINOR extension (SizingResult DTO), but the actual
commit for L17 (task-2452) did not make that extension, and this leaf's
(L19, task-2525) file scope is pinned to `sizing/**` — so the sizing package
owns its own public result type and shared exception directly here (it does
not touch `models.py`, which is out of scope).
"""
...
class PortfolioSizingError(ValueError):
    """Sizing input violated the contract — fail-closed base exception."""

    code = "PORTFOLIO_SIZING_ERROR"


class SizingInputMissingError(PortfolioSizingError):
    """Required input is `None` — reject instead of substituting 0/a default."""
...
class SizingInputInvalidError(PortfolioSizingError):
    """Input value is present but outside the range the formula defines (e.g. `price <= 0`)."""
...
def require(value: _T | None, field: str) -> _T:
    """Filter `None` inputs here so each formula file doesn't repeat this check."""
    if value is None:
...
def hash_inputs(method: SizingMethod, **fields: Any) -> str:
    """R1: the same formula and same inputs always yield the same `inputs_hash`."""
