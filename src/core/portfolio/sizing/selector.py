...
Dispatches method → function + validates result `inputs_hash`. Rejects
immediately when `config.method` and `inp.portfolio_config.method` disagree
(caller bug), and similarly rejects when the returned
`SizingResult.method` differs from the requested method or when `inputs_hash`
is not a valid sha256 hex digest (implementation error / tampering).
"""
