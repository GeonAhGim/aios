...
`f* = p − (1−p)/b`, `f = min(max(f*, 0), kelly_cap)`. `win_rate`(p) is a [0,1]
probability, `avg_win_loss_ratio`(b) is the average win/loss ratio — reject outright
rather than substituting 0/1 etc. if either is `None`.
"""
