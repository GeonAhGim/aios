...
def assert_supported(profile: VenueCapabilityProfile, cmd: SubmitOrderCommand) -> None:
    # EM-19(task-1751) -- trigger_price/trailing_offset are wired into the
    # contract/command_digest (§9 EM-19), but no leaf yet connects the actual
    # trigger evaluation (domain/order_types/*) to this execution path
    # (submit_order.py -> outbox -> venue). Silently accepting it before that
    # wiring exists would send a "stop order" out immediately as market/limit
    # (I-10 violation, a silently ignored trigger) -- reject fail-closed
