"""L4-14/L4-31 — dispatcher shared write primitives: outbox fence + order
transition + payload restoration.

Spec: docs/specs/L4_execution_oms_and_exchange_v1.0.md §5.1 (outbox
done/retry/dead fence), §4.4 (outbox state-machine table — `done`/`dead`/
`defer`/`retry_or_dead` each implement §4.4's `DONE`/`DEAD`/`RETRY`
(including `not_before` deferral) transitions), §5.4 (backoff/max_attempts),
§7.3 (payload_hash — raw storage prohibited). §2-C/§9 L4-31: this file is a
module split out of `outbox_dispatcher.py` (L4-14), retroactively
registering a row the spec did not have (ADR-2026-09-06-G §10).
