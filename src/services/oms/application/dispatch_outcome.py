"""L4-14/L4-31 — outbox dispatcher's exchange response/exception -> outcome
classification (pure, no I/O).

Spec: docs/specs/L4_execution_oms_and_exchange_v1.0.md §3.4, §5.4, §6
F3/F4/F14, §4.2/§4.4 (outbox state-machine table — this module's
`OutcomeKind` jointly determines the §4.4 outbox row state transition and
the trigger for the §4.2 order state transition). §2-C/§9 L4-31: this file
is a pure-determination module split out of `outbox_dispatcher.py` (L4-14),
