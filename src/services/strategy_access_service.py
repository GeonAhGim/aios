...
생기는 구멍을 막기 위해서다. FD-18.5b(결제 확인 API)는 아직 없어(작업
트리 순서상 뒤) 실제로 CONFIRMED 전이는 지금 아무 경로로도 일어나지
않는다 — 이는 버그가 아니라 현재 시스템이 실제로 할 수 있는 것을
정직하게 반영한 상태다.

10.3-B 블랙박스 원칙: 이 서비스는 strategies/strategy_purchases/
strategy_listings만 조회하고 구매자별 실행 상태(FD-16 소관, 아직 없음)는
전혀 다루지 않는다 — 판매자가 구매자의 실행 데이터를 볼 경로 자체가
없다.
