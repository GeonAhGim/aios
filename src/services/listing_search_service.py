...
기본 정렬은 리스팅 생성일이 아니라 검증통과일(verified_at, FD-13.2 완료
시각) 역순 — 판매자가 재등록으로 상단 노출을 조작하는 것을 막는다
(검증통과일은 검증담당자를 거쳐야만 갱신되므로 조작이 어렵다). 동점 시
2차 정렬은 샤프비율 내림차순(NULL은 항상 마지막).
        ...
        *,
        ...
        async with self._pool.acquire() as conn:
            ...
            rows = await conn.fetch(
                ...
                WHERE {where_clause}
                ORDER BY l.verified_at DESC NULLS LAST, l.sharpe_ratio DESC NULLS LAST
