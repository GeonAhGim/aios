...
of the request/response field names.

Split into a separate file from `market_data_mixin.py` (get_ticker/
get_orderbook/get_ohlcv/subscribe_ticker_stream, the `ExchangeAdapter` ABC
contract methods) per §7 line-cap policy — market_data_mixin.py was already
219 lines and these 9 endpoints would push it well past the 300-line warn
threshold. These are NH-only read extension methods outside the ABC
contract, same split rationale as `credit_reserved_mixin.py` vs
`trading_mixin.py`. No `@require_paper_sandbox` guard is needed — these are
read-only quote lookups, not order-placing calls.
"""
...
class NHQuoteExtraMixin:
    ...
    async def get_etf_current(self: NHHTTPClient, symbol: str) -> EtfCurrentQuote:
        ...
        ⚠️ The official snapshot notes example responses carry undocumented
