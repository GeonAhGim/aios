...
class BitgetCopyTradingMixin:
    ...
    async def unfollow_copy_trader(self: SignedRequestClient, trader_id: str) -> bool:
        raw = await self._request(
    ...
    async def get_copy_trading_current_orders(
        self: SignedRequestClient, *, trader_id: str | None = None
    ) -> list[dict[str, Any]]:
        params: dict[str, Any] = {}
    ...
    async def get_copy_trading_order_history(
        self: SignedRequestClient, *, trader_id: str | None = None, limit: int = 100
    ) -> list[dict[str, Any]]:
    ...
    async def get_copy_trading_followers(
        self: SignedRequestClient,
        *,
        limit: int = 100,
    ) -> list[dict[str, Any]]:
    ...
    async def update_copy_trading_trader_profile(
        self: SignedRequestClient,
        **settings: Any,
    ) -> dict[str, Any]:
    ...
    async def get_copy_trading_profit_summary(self: SignedRequestClient) -> dict[str, Any]:
        raw = await self._request(
            "GET", "/api/v2/copy/mix-trader/order-profit-history-summary"
        )
        return dict(raw["data"])
