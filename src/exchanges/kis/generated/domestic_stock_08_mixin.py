"""BR-12 자동 생성(ADR-2026-09-06-I D7) -- domestic_stock 미착수 TR 청크 08.
...
class KISGeneratedDomesticStock08Mixin(_KISWsHost):
    ...
    async def program_trade_krx_h0stpgm0(
        ...
        *,
        ...
        [국내주식] 실시간시세 > 국내주식 실시간프로그램매매 (KRX)  [실시간-048] -- WS
        tr_id=H0STPGM0. 필수 파라미터: tr_key. 응답 필드: mksc_shrn_iscd, stck_cntg_hour,
        seln_cnqn, seln_tr_pbmn, shnu_cnqn, shnu_tr_pbmn, ntby_cnqn, ntby_tr_pbmn, seln_rsqn,
        shnu_rsqn, whol_ntby_qty.
        """
    ...
    async def exp_ccnl_total_h0unanc0(
        ...
        *,
        ...
        """BR-12 생성(ADR-2026-09-06-I D7). 미검증: 실계좌 왕복 미확인.
        [국내주식] 실시간시세 > 국내주식 실시간예상체결(통합) -- WS tr_id=H0UNANC0. 필수
    ...
    async def asking_price_total_h0unasp0(
        ...
        *,
        ...
        """BR-12 생성(ADR-2026-09-06-I D7). 미검증: 실계좌 왕복 미확인.
        [국내주식] 실시간시세 > 국내주식 실시간호가 (통합) -- WS tr_id=H0UNASP0. 필수 파라미터:
    ...
    async def ccnl_total_h0uncnt0(
        ...
        *,
        ...
        """BR-12 생성(ADR-2026-09-06-I D7). 미검증: 실계좌 왕복 미확인.
        [국내주식] 실시간시세 > 국내주식 실시간체결가 (통합) -- WS tr_id=H0UNCNT0. 필수
        파라미터: tr_key. 응답 필드: MKSC_SHRN_ISCD, STCK_CNTG_HOUR, STCK_PRPR, PRDY_VRSS_SIGN,
        PRDY_VRSS, PRDY_CTRT, WGHN_AVRG_STCK_PRC, STCK_OPRC, STCK_HGPR, STCK_LWPR, ASKP1, BIDP1,
        CNTG_VOL, ACML_VOL, ACML_TR_PBMN, SELN_CNTG_CSNU, SHNU_CNTG_CSNU, NTBY_CNTG_CSNU, CTTR,
        SELN_CNTG_SMTN, SHNU_CNTG_SMTN, CNTG_CLS_CODE, SHNU_RATE, PRDY_VOL_VRSS_ACML_VOL_RATE,
    ...
    async def member_total_h0unmbc0(
        ...
        *,
        ...
        """BR-12 생성(ADR-2026-09-06-I D7). 미검증: 실계좌 왕복 미확인.
        [국내주식] 실시간시세 > 국내주식 실시간회원사 (통합) -- WS tr_id=H0UNMBC0. 필수
        파라미터: tr_key. 응답 필드: MKSC_SHRN_ISCD, SELN2_MBCR_NAME1, SELN2_MBCR_NAME2,
    ...
    async def market_status_total_h0unmko0(
        ...
        *,
        ...
        """BR-12 생성(ADR-2026-09-06-I D7). 미검증: 실계좌 왕복 미확인.
        [국내주식] 실시간시세 > 국내주식 장운영정보(통합) -- WS tr_id=H0UNMKO0. 필수 파라미터:
        tr_key. 응답 필드: TRHT_YN, TR_SUSP_REAS_CNTT, MKOP_CLS_CODE, ANTC_MKOP_CLS_CODE,
