"""entities top-level re-export surface for cross-aggregate consumers.

Spec: task-5418 (task-5404 분할 4/4), `.importlinter` boundary:foundation-aggregates.
Other foundation aggregates may not import `entities.domain.*` or
`entities.adapters.*` directly (교차 애그리게잇 경계) — they import the
...
`entities.domain.X` to `entities.api` is nil.
...
__all__ = ["default_entity_id", "default_fund_id", "default_portfolio_id"]
