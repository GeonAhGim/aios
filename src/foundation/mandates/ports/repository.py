...
class MandateRepository(Protocol):
    ...
    async def get_bundle(self, bundle_id: UUID) -> PolicyBundle | None:
        """Looks up directly by `bundle_id` — `get_bundle_for_revision` is for
        callers who already know the revision, while callers holding only
        `policy_decision.bundle_id` (CM-13 `application/explain.py`) dereference it here."""
        ...
    ...
    async def get_policy_decision(self, decision_id: UUID) -> PolicyDecision | None:
        """Looks up directly by `id` — `get_cached_decision` is a cache lookup that
        finds "a decision currently reusable" via fingerprint+TTL, whereas this
        method is a replay lookup that reads back one specific decision made in
        the past as-is (CM-13 `explain()`), so no TTL/expiry applies (WORM: same row every read)."""
        ...
