...
def _cash_delta(trade: ProposedTrade) -> Decimal:
    """BUY drains cash, SELL returns cash."""
    if trade.side == "BUY":
...
def _gross_notional_delta(trade: ProposedTrade) -> Decimal:
    """BUY adds new notional to gross (ABS increases); SELL reduces gross by
    the notional being closed."""
    return trade.notional if trade.side == "BUY" else -trade.notional


def _per_symbol_pct_delta(
    before: PortfolioAggregate, trade: ProposedTrade, new_equity: Decimal
) -> dict[str, Decimal]:
    per_symbol_pct_delta: dict[str, Decimal] = {}
    ...
    return per_symbol_pct_delta


def _per_strategy_pct_delta(
    before: PortfolioAggregate, trade: ProposedTrade, new_equity: Decimal
) -> dict[str, Decimal]:
    per_strategy_pct_delta: dict[str, Decimal] = {}
    if trade.strategy_id is None:
        return per_strategy_pct_delta
...
def _cash_and_exposure_pct_delta(
    ...
    return cash_pct_delta, total_exposure_pct_delta
...
def delta_impact(
    *,
    ...
    cash_delta = _cash_delta(trade)

    # Total equity delta: exposure changes by exposure_delta, cash changes by
    ...
    # BUY adds one open position (symbol didn't exist or was flat); SELL
    # removes one (symbol goes flat). Simplification: always ±1.
    position_count_delta = 1 if trade.side == "BUY" else -1

    gross_notional_delta = _gross_notional_delta(trade)
    ...
    per_symbol_pct_delta = _per_symbol_pct_delta(before, trade, new_equity)
    per_strategy_pct_delta = _per_strategy_pct_delta(before, trade, new_equity)
    cash_pct_delta, total_exposure_pct_delta = _cash_and_exposure_pct_delta(
        before, cash_delta, gross_notional_delta, new_equity
    )
