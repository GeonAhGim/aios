"""performance top-level re-export surface for cross-aggregate consumers.

Spec: task-5418 (task-5404 분할 4/4), `.importlinter` boundary:foundation-aggregates.
Other foundation aggregates may not import `performance.domain.*` or
`performance.adapters.*` directly (교차 애그리게잇 경계) — they import the
...
`performance.domain.X` to `performance.api` is nil.
