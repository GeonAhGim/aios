...
`source_hash` — a mismatch means the recomputed result differs from the previous one,
so raises `NavChainBrokenError` (POS_NAV_CHAIN_BROKEN, no overwrite, requires operations intervention).
